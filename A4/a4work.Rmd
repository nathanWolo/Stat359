...
# von_bertalanffy <- function(time, a, b, c) a - a * exp(-b * (time + c))
...
To provide a 95% confidence interval for lim(t→∞)f(t) for each of the three models, we will first need to extract the parameter estimates and their standard errors from each model. The lim(t→∞)f(t) for each model is as follows:

Logistic: lim(t→∞)f(t) = a

Gompertz: lim(t→∞)f(t) = a

Von Bertalanffy: lim(t→∞)f(t) = a

For each model, the limit as t goes to infinity is the 'a' parameter.

Now let's calculate the 95% confidence intervals for each model:
...
# 95% confidence intervals for lim(t→∞)f(t) for each model
...
These confidence intervals represent the range within which we can be 95% confident that the true value of lim(t→∞)f(t) lies. In the context of this problem, this represents the height the tree would reach as time goes to infinity, or in other words, the maximum height the tree can grow to.
